...
import { Upload, CheckCircle2, AlertTriangle, FileText, HelpCircle, Box, Wrench } from 'lucide-react';
import Modal from '../Modal';
import { useHelp } from '../../contexts/HelpContext';
import { MaterialImportResult } from '../../materials/services/materialImportService';
...
export const MaterialImportModal: React.FC<MaterialImportModalProps> = ({
    ...
    const handleShowFormatHelp = () => {
        setHelpContent({
            title: "Importação de Catálogo - Formato Excel",
            content: `
                ...
                <ul class="list-disc pl-4 space-y-1 text-sm">
                    ...
                    <li><strong>Tipo</strong>: 'Material' ou 'Serviço'. Se contiver 'mão' ou 'serv', assume Serviço.</li>
                    <li><strong>Código / Ref</strong>: Se vazio, será gerado automaticamente.</li>
                    <li><strong>Preço / Valor</strong></li>
                    <li><strong>Unidade</strong>: Un, Kg, h, etc.</li>
                    <li><strong>Stock / Qtd</strong>: Quantidade inicial.</li>
                    <li><strong>Stock Mínimo / Alerta</strong>: Nível de alerta.</li>
                    <li><strong>Obs / Notas</strong></li>
                </ul>
            `
        });
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Importar Catálogo (Excel)">
            <div className="flex flex-col h-[85vh]">
                            ...
                            <table className="min-w-full text-xs whitespace-nowrap">
                                <thead className="bg-gray-100 sticky top-0 font-bold text-gray-500 uppercase z-10">
                                    <tr>
                                        ...
                                        <th className="p-3 text-right border-b">Preço</th>
                                ...
                                <tbody className="divide-y divide-gray-200 bg-white">
                                    {result.drafts.map((d, i) => (
                                        <tr key={i} className="hover:bg-gray-50">
                                            <td className="p-3">
                                                {d.type === 'Material' ? (
                                                    <span className="bg-blue-50 text-blue-600 px-2 py-1 rounded-lg font-black uppercase flex items-center gap-1 w-fit"><Box size={10}/> Mat</span>
                                                ) : (
                                                    <span className="bg-purple-50 text-purple-600 px-2 py-1 rounded-lg font-black uppercase flex items-center gap-1 w-fit"><Wrench size={10}/> Svc</span>
                                                )}
                                            </td>
                                            <td className="p-3 font-mono text-gray-500">{d.internalCode || '(Auto)'}</td>
                                            <td className="p-3 font-bold text-gray-800">{d.name}</td>
                                            <td className="p-3 text-right font-mono">{d.price?.toLocaleString()} CVE</td>
                                            <td className="p-3 text-center">{d.stock || '-'}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
                ...
                <div className="pt-4 border-t flex justify-between items-center shrink-0">
                    ...
                    <div className="flex gap-3">
                        ...
                        >
                            <Upload size={18}/> Importar {result.drafts.length} Itens
                        </button>
                    </div>
                </div>
            </div>
